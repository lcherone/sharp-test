    ...
    var request = function (action, method, data) {
        ...
        return $.ajax({
            ...
            complete: function (xhr, textStatus) {
                /*
                var token = xhr.getResponseHeader('usertoken');
                if (token) {
                    store.set('token', xhr.getResponseHeader('token'));
                } else {
                    if (xhr.status === 200) {
                        console.error('[' + method + '] ' + action);
                        console.error('[bug] No tokens in xhr response', xhr);
                    } else {
                        console.log('[' + method + '] ' + action, xhr);
                    }
                }
                */
            }
